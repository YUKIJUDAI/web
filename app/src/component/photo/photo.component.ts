...
class Item {
    ...
    tilt(ev) {
        ...
        for (let key in this.DOM.animatable) {
            ...
            window["TweenMax"].to(this.DOM.animatable[key], 1.5, {
                ease: "Power1.easeOut",
...
class Overlay {
    ...
    show(contentItem) {
        ...
        // show revealer
        window["TweenMax"].to(this.DOM.reveal, 0.5, {
    ...
    revealItem() {
        ...
        for (let el of itemElems) {
            ...
            window["TweenMax"].to(el, lineEq(0.8, 1.2, win.width, 0, Math.abs(bounds.left + bounds.width - win.width)), {
                ease: "Expo.easeOut",
                delay: 0.2,
                startAt: {
...
class Grid {
    ...
    constructor(el) {
        this.DOM = { el: el };
        this.items = [];
        Array.from(this.DOM.el.querySelectorAll("a.grid__item")).map((item: any) => {
